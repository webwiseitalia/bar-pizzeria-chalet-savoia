...
export default function Stagioni() {
  ...
  return (
    <section ref={sectionRef} id="stagioni" className="relative overflow-hidden texture-noise texture-paper" style={{ padding: 'var(--space-section) 0', background: 'var(--color-cream)' }}>
      {/* Subtle warm gradient blobs */}
      <div className="absolute top-0 left-[15%] w-96 h-96 rounded-full opacity-[0.05] pointer-events-none blur-3xl" style={{ background: 'var(--color-fire)' }} />
      <div className="absolute bottom-20 right-[10%] w-64 h-64 rounded-full opacity-[0.04] pointer-events-none blur-3xl" style={{ background: 'var(--color-burgundy)' }} />
      <div className="relative z-10" style={{ padding: '0 var(--space-gutter)' }}>
        <div className="season-text mb-14 md:mb-20 text-center">
          ...
          <h2 className="display-lg" style={{ color: 'var(--color-wood-900)' }}>
            Inverno <span style={{ color: 'var(--color-fire)' }}>&</span> <em style={{ color: 'var(--color-wood-600)' }}>Estate</em>
          </h2>
        </div>
          ...
          <div className="lg:col-span-7 lg:col-start-1 relative z-10">
            <div className="season-img-winter relative overflow-hidden" style={{ height: 'clamp(350px, 50vw, 580px)', boxShadow: '0 16px 50px rgba(23,37,84,0.2)' }}>
              ...
              <div className="absolute bottom-0 left-0 right-0 p-6 md:p-10">
                ...
                <p className="text-white/75 mt-3 max-w-md body-text">Colazione prima delle piste, pranzo veloce o completo, après-ski con bombardino e birra. Atmosfera baita con stufe, legno, calore.</p>
                <div className="flex flex-wrap gap-3 mt-5">
                  {['Colazione', 'Pranzo', 'Après-ski', 'Bombardino', 'Forno a legna'].map(tag => (
                    <span key={tag} className="text-white/60 border border-white/20 px-3 py-1" style={{ fontFamily: 'var(--font-accent)', fontSize: '0.6rem', letterSpacing: '0.08em', textTransform: 'uppercase' }}>{tag}</span>
                  ))}
                </div>
              </div>
            </div>
          ...
          <div className="lg:col-span-6 lg:col-start-7 lg:-mt-16 lg:pt-32 relative z-20">
            <div className="season-img-summer relative overflow-hidden" style={{ height: 'clamp(300px, 40vw, 480px)', boxShadow: '0 16px 50px rgba(120,53,15,0.2)' }}>
              ...
              <div className="absolute bottom-0 left-0 right-0 p-6 md:p-10">
                ...
                <p className="text-white/75 mt-3 max-w-md body-text">Sosta per motociclisti, ciclisti ed escursionisti. Terrazza al sole, birre fresche. Base per il Ghiacciaio Presena e i forti della Grande Guerra.</p>
                <div className="flex flex-wrap gap-3 mt-5">
        ...
        <div className="mt-16 md:mt-24 flex flex-wrap gap-12 md:gap-20 md:ml-[8%]">
          {[
            ...
            <div key={i} className="season-stat">
              ...
              <span className="text-sm ml-1" style={{ color: 'var(--color-wood-600)' }}>{stat.unit}</span>
              <span className="block mt-1" style={{ fontFamily: 'var(--font-accent)', fontSize: '0.6rem', letterSpacing: '0.15em', textTransform: 'uppercase', color: 'var(--color-wood-600)' }}>{stat.label}</span>
            </div>
          ))}
        </div>
      </div>
